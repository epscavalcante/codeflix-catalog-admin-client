...
async function submit() {
  try {
    const res = await genreGateway.update(route.params.id as any, {
...
onMounted(async () => {
  ...
  const genreResponse = await genreGateway.find(route.params.id as any);
  genre.value.name = genreResponse.name;
  genre.value.categoriesId = genreResponse.categoriesId as any;
  await getCategories();
});
</script>
